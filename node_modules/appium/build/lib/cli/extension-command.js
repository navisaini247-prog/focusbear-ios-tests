...
class ExtensionCliCommand {
    ...
    async _run({ installSpec, scriptName, extraArgs = [], bufferOutput = false }) {
        ...
        if (!scriptName) {
            ...
            const existingScripts = await bluebird_1.default.filter(allScripts, async ([, p]) => await support_1.fs.exists(node_path_1.default.join(root, p)));
...
async function injectAppiumSymlink(dstFolder, logger) {
    ...
    try {
        ...
        if (await support_1.fs.exists(dstFolder) && !(await support_1.fs.exists(symlinkPath))) {
            await support_1.fs.symlink(appiumModuleRoot, symlinkPath, support_1.system.isWindows() ? 'junction' : 'dir');
        }
    }
    catch (error) {
        // This error is not fatal, we may still doing just fine if the module being loaded is a CJS one
        logger.info(`Cannot create a symlink to the appium module '${appiumModuleRoot}' in '${dstFolder}'. ` +
