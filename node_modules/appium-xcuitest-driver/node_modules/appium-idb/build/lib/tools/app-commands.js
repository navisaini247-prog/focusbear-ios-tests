...
appCommands.listApps = async function listApps() {
    logger_js_1.default.debug(`Listing the info about installed apps on the device '${this.udid}'`);
    return (0, helpers_1.fixOutputToArray)(await this.exec(['list-apps'], ['--json']));
};
/**
...
appCommands.installApp = async function installApp(appPath) {
    logger_js_1.default.debug(`Installing '${appPath}' to the device '${this.udid}'`);
...
appCommands.launchApp = async function launchApp(bundleId, opts = {}) {
    const { foregroundIfRunning = true, wait = false } = opts;
    logger_js_1.default.debug(`Launching '${bundleId}' on the device '${this.udid}'`);
    const args = [];
...
appCommands.terminateApp = async function terminateApp(bundleId) {
    logger_js_1.default.debug(`Terminating '${bundleId}' on the device '${this.udid}'`);
...
appCommands.uninstallApp = async function uninstallApp(bundleId) {
    logger_js_1.default.debug(`Uninstalling '${bundleId}' from the device '${this.udid}'`);
