...
function toInternalServiceName(serviceName) {
    const lowerName = lodash_1.default.toLower(serviceName);
    if (lodash_1.default.has(SERVICES, lowerName)) {
...
function formatStatus(status) {
    return status === STATUS.UNSET || status === STATUS.NO ? lodash_1.default.toUpper(status) : status;
}
/**
...
async function getAccess(bundleId, serviceName) {
    ...
    const dbPath = node_path_1.default.resolve(this.getDir(), 'Library', 'TCC', 'TCC.db');
