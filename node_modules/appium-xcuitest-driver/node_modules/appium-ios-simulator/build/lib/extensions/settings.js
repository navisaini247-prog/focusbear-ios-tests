...
async function updateSettings(domain, updates) {
    ...
    await bluebird_1.default.all(argChunks.map((args) => this.simctl.spawnProcess(['defaults', 'write', domain, ...args])));
    return true;
}
/**
...
async function configureLocalization(opts = {}) {
    ...
    if (globalPrefs.AppleLanguages) {
        const absolutePrefsPath = node_path_1.default.join(this.getDir(), 'Library', 'Preferences', GLOBAL_PREFS_PLIST);
        try {
    ...
    await bluebird_1.default.all(argChunks.map((args) => this.simctl.spawnProcess(['defaults', 'write', GLOBAL_PREFS_PLIST, ...args])));
    if (keyboard && keyboardId) {
        ...
        await bluebird_1.default.all(argChunks.map((args) => this.simctl.spawnProcess(['defaults', 'write', 'com.apple.Preferences', ...args])));
    }
    if (globalPrefs.AppleLanguages) {
        if (lodash_1.default.isEqual(previousAppleLanguages, globalPrefs.AppleLanguages)) {
            ...
            await bluebird_1.default.all(SERVICES_FOR_TRANSLATION.map((arg) => this.simctl.spawnProcess(['launchctl', 'stop', arg])));
        }
    }
    return true;
}
...
async function updatePreferences(devicePrefs = {}, commonPrefs = {}) {
    ...
    const plistPath = node_path_1.default.resolve(homeFolderPath, 'Library', 'Preferences', 'com.apple.iphonesimulator.plist');
...
function compileSimulatorPreferences(opts = {}) {
    ...
    const devicePreferences = opts.devicePreferences
        ? lodash_1.default.cloneDeep(opts.devicePreferences)
        : {};
    if (scaleFactor) {
...
function verifyDevicePreferences(prefs = {}) {
    ...
    if (!lodash_1.default.isUndefined(prefs.SimulatorWindowCenter)) {
        ...
        if (!lodash_1.default.isString(prefs.SimulatorWindowCenter) ||
    ...
    if (!lodash_1.default.isUndefined(prefs.SimulatorWindowOrientation)) {
        ...
        if (!prefs.SimulatorWindowOrientation ||
