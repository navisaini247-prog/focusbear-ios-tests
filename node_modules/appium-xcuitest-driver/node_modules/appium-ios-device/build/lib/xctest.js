...
exports.Xctest = void 0;
const services_1 = require("./services");
const instrument_1 = require("./instrument");
const headers_1 = require("./instrument/headers");
const nskeyed_1 = require("./instrument/transformer/nskeyed");
const testmanagerd_1 = require("./testmanagerd");
const support_1 = require("@appium/support");
const utilities_1 = require("./utilities");
const logger_1 = require("./logger");
const bluebird_1 = __importDefault(require("bluebird"));
const { DAEMON_CONNECTION_INTERFACE } = testmanagerd_1.TESTMANAGERD_CHANNEL;
...
class Xctest {
    ...
    async _launchAppRunner(majorVersion, sessionIdentifier) {
        const { PROCESS_CONTROL } = instrument_1.INSTRUMENT_CHANNEL;
        const installation = await (0, services_1.startInstallationProxyService)(this.udid);
        let lookupResult;
        try {
        ...
        logger_1.log.info(`SignerIdentifier: ${signerIdentifier}`);
        ...
        const xctestConfiguration = new nskeyed_1.XCTestConfiguration({
            ...this._conf,
        ...
        this._instrumentService = await (0, services_1.startInstrumentService)(this.udid);
        ...
        logger_1.log.info(`Pid of launched ${this.xctestBundleId}: ${pid}`);
        const msg = new headers_1.DTXMessageAuxBuffer();
    ...
    async _writeConfigurationToDevice(xctestConfiguration, xctestPath) {
        ...
        const houseArrestService = await (0, services_1.startHouseArrestService)(this.udid);
        let vendContainer;
        let stream;
        try {
            ...
            for (const file of list) {
                if (file.endsWith(XCTEST_CONFIGURATION_EXTENSION)) {
                    ...
                    logger_1.log.debug(`removing ${fullPath}`);
    ...
    async _startInitialSession(majorVersion) {
        this._initialControlSession = await (0, services_1.startTestmanagerdService)(this.udid);
    ...
    async _startExecSession(sessionIdentifier) {
        this._execTestPlanSession = await (0, services_1.startTestmanagerdService)(this.udid);
        ...
        const showLogMessage = (message) => {
            if (message.auxiliaries.join('').includes('Received test runner ready reply with error: (null')) {
                logger_1.log.info('Test runner ready');
        ...
        const msg = new headers_1.DTXMessageAuxBuffer();
        msg.appendObject(new nskeyed_1.NSUUID(sessionIdentifier));
    ...
    async start() {
        if (this.running) {
            ...
            logger_1.log.info(`${message} Doing nothing here`);
            return;
        }
        this.running = true;
        try {
            const productVersion = await (0, utilities_1.getOSVersion)(this.udid);
            const majorVersion = parseInt(productVersion.split('.')[0], 10);
            const sessionIdentifier = support_1.util.uuidV4();
            //first connection
    ...
    stop() {
        ...
        logger_1.log.debug(message);
    }
}
exports.Xctest = Xctest;
exports.default = Xctest;
