...
const webinspector_decoder_1 = require("./transformer/webinspector-decoder");
const webinspector_encoder_1 = require("./transformer/webinspector-encoder");
const plist_service_decoder_1 = require("../plist-service/transformer/plist-service-decoder");
const plist_service_encoder_1 = require("../plist-service/transformer/plist-service-encoder");
const length_based_splitter_1 = require("../util/transformer/length-based-splitter");
const stream_logger_1 = require("../util/transformer/stream-logger");
...
class WebInspectorService extends base_service_1.BaseServiceSocket {
     ...
     */
    constructor(opts = {}) {
        const { majorOsVersion, isSimulator = false, socketChunkSize, verbose = false, verboseHexDump = false, socketClient, maxFrameLength = (0, utilities_1.getMaxFrameLength)(MAX_FRAME_SIZE), } = opts;
        super(socketClient);
        // set the largest fragment size for the socket, if the option is there
        if (lodash_1.default.isFunction(socketClient.setMaxSendFragment) &&
            lodash_1.default.isNumber(socketChunkSize) &&
            socketChunkSize > 0) {
            if (socketClient.setMaxSendFragment(socketChunkSize)) {
                logger_1.log.debug(`Maximum TLS fragment size set to '${socketChunkSize}'`);
            }
            else {
                // anything over the _actual_ maximum will fail, and things will remain the same
                logger_1.log.warn(`Unable to set TLS fragment size to '${socketChunkSize}'`);
            }
        }
        ...
        if (!isSimulator &&
            lodash_1.default.isNumber(majorOsVersion) &&
    ...
    _initializeFullMessageSupport(verbose, maxFrameLength) {
        this._decoder = new plist_service_decoder_1.PlistServiceDecoder();
        this._socketClient
            // log first, in case there is a problem in processing
            .pipe(new stream_logger_1.StreamLogger(stream_logger_1.StreamLogger.RECEIVE, verbose))
            .pipe((this._splitter = new length_based_splitter_1.LengthBasedSplitter({
            ...
            .pipe(this._decoder);
        this._encoder = new plist_service_encoder_1.PlistServiceEncoder();
        this._encoder.pipe(new stream_logger_1.StreamLogger(stream_logger_1.StreamLogger.SEND, verbose)).pipe(this._socketClient);
    }
    /**
    ...
    _initializePartialMessageSupport(verbose, maxFrameLength) {
        ...
        this._decoder = new webinspector_decoder_1.WebInspectorDecoder(constants_1.MB);
        this._socketClient
            // log first, in case there is a problem in processing
            .pipe(new stream_logger_1.StreamLogger(stream_logger_1.StreamLogger.RECEIVE, verbose))
            .pipe((this._splitter = new length_based_splitter_1.LengthBasedSplitter({
            ...
            .pipe(new plist_service_decoder_1.PlistServiceDecoder())
            .pipe(this._decoder);
        this._encoder = new webinspector_encoder_1.WebInspectorEncoder();
        this._encoder
            .pipe(new plist_service_encoder_1.PlistServiceEncoder())
            .pipe(new stream_logger_1.StreamLogger(stream_logger_1.StreamLogger.SEND, verbose))
    ...
    sendMessage(rpcObject) {
        ...
        if (this._verbose || lastError) {
            logger_1.log.debug('Sent message to Web Inspector:');
            logger_1.log.debug(support_1.util.jsonStringify(message, null));
            if (!lodash_1.default.isEqual(message, rpcObject)) {
                logger_1.log.debug('Original message:');
                logger_1.log.debug(support_1.util.jsonStringify(rpcObject, null));
            }
        }
        if (lastError) {
            throw lastError;
    ...
    listenMessage(onData) {
        ...
        this._decoder.on('data', (data) => {
            if (this._verbose) {
                logger_1.log.debug('Received message from Web Inspector:');
                logger_1.log.debug(support_1.util.jsonStringify(data, null));
            }
            onData(data);
        });
    }
