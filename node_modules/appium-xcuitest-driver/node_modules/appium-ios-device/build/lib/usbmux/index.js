...
const usbmux_decoder_js_1 = require("./transformer/usbmux-decoder.js");
const usbmux_encoder_js_1 = require("./transformer/usbmux-encoder.js");
const node_path_1 = __importDefault(require("node:path"));
const plist_service_1 = require("../plist-service");
const lockdown_1 = require("../lockdown");
...
try {
    ...
    ({ name, version } = require(node_path_1.default.resolve(__dirname, '..', '..', '..', 'package.json')));
}
catch {
...
async function getDefaultSocket(opts = {}) {
    ...
    const hasOpts = Object.values(opts).some((v) => !lodash_1.default.isNil(v));
    ...
    const hasExplicitHostPort = !!(opts.socketHost ||
        opts.socketPort ||
        envOpts.socketHost ||
        envOpts.socketPort);
    const isWinOrWsl = process.platform === 'win32' ||
        (process.platform === 'linux' && /microsoft/i.test(node_os_1.default.release()));
    const preferPath = hasExplicitPath || !(hasExplicitHostPort || isWinOrWsl);
    let socket;
    if (preferPath) {
        const pathExists = await support_1.fs.exists(socketPath);
        if (!pathExists) {
            throw new Error(`The usbmuxd socket path '${socketPath}' does not exist or is not accessible`);
        }
        socket = node_net_1.default.createConnection(socketPath);
    }
    else {
        socket = node_net_1.default.createConnection(socketPort, socketHost);
    }
...
function getSocketOptionsFromEnv() {
    ...
    logger_1.log.debug(`Using USBMUXD_SOCKET_ADDRESS environment variable as default socket: ${usbmuxdSocketAddress}`);
...
class Usbmux extends base_service_1.BaseServiceSocket {
    constructor(socketClient) {
        ...
        this._encoder = new usbmux_encoder_js_1.UsbmuxEncoder();
    ...
    _handleData(data) {
        const cb = this._responseCallbacks[data.header.tag] || lodash_1.default.noop;
    ...
    async readPairRecord(udid, timeout = 5000) {
        const { tag, receivePromise } = this._receivePlistPromise(timeout, (data) => {
            ...
            try {
                return support_1.plist.parsePlist(data.payload.PairRecordData);
            }
            catch {
                throw new Error(`Unexpected data: ${JSON.stringify(data)}`);
            }
    ...
    _receivePlistPromise(timeout = 5000, responseCallback) {
        const tag = this._tag++;
        const receivePromise = new bluebird_1.default((resolve, reject) => {
    ...
    async findDevice(udid, timeout = 5000) {
        ...
        return lodash_1.default.find(devices, (device) => device.Properties.SerialNumber === udid);
    }
    /**
