...
const { exists, readdir, mkdir, rimraf } = support_1.fs;
/**
...
async function listGithubImageList(githubImageOption) {
    ...
    for (let i = 0; i <= subFolderList.length; i++) {
        const res = await axios_1.default.get(curUrl);
        /**
        ...
        const nextItem = lodash_1.default.find(treeList, (item) => item.path === entry);
        if (!nextItem) {
...
async function findDeveloperImageFromDirectory(entry) {
    ...
    for (const subEntry of fileList) {
        ...
        if (subEntry.isDirectory()) {
            const fullPath = (0, node_path_1.join)(entry, subEntry.name);
...
async function searchAndDownloadDeveloperImageFromGithub(finalVersion, fullDownloadPath, githubImageOption) {
    const fileNameRegExp = new RegExp(`${lodash_1.default.escapeRegExp(finalVersion)}(\\(([\\w_|.()])+\\))?.zip`);
    const { githubRepo, subFolderList = [], branch = 'master' } = githubImageOption;
    /** @type {string | undefined} */
    let fileUrl;
    ...
    const targetFile = lodash_1.default.find(fileList, (item) => fileNameRegExp.test(item.path));
    ...
    try {
        logger_1.log.info(`Downloading developer image for ${finalVersion} to ${fullDownloadPath} from ${fileUrl}`);
        await support_1.net.downloadFile(fileUrl, fullDownloadPath);
    }
    catch (e) {
...
//# sourceMappingURL=list_developer_image.js.map