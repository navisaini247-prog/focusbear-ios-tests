...
exports.XCTestConfiguration = exports.NSDate = exports.NSUUID = exports.NSURL = exports.BaseArchiveHandler = void 0;
exports.updateNSKeyedArchiveClass = updateNSKeyedArchiveClass;
exports.unarchive = unarchive;
exports.archive = archive;
const bplist_parser_1 = __importStar(require("bplist-parser"));
const bplist_creator_1 = __importDefault(require("bplist-creator"));
const parse_1 = require("../../util/uuid/parse");
const stringify_1 = require("../../util/uuid/stringify");
const lodash_1 = __importDefault(require("lodash"));
const node_util_1 = require("node:util");
const logger_1 = require("../../logger");
const NSKEYED_ARCHIVE_VERSION = 100_000;
// @ts-ignore UID is not exposed to typedefs
const NULL_UID = new bplist_parser_1.default.UID(0);
...
class NSUUID extends BaseArchiveHandler {
    ...
    decodeArchive(archive) {
        return (0, stringify_1.stringify)(archive.decode('NS.uuidbytes'));
    }
    encodeArchive(obj, archive) {
        archive._archiveObj['NS.uuidbytes'] = Buffer.from((0, parse_1.parse)(obj._data).buffer);
    }
}
...
class Unarchive {
    ...
    unpackArchiveHeader() {
        const plist = (0, bplist_parser_1.parseBuffer)(this.input)[0];
        const createPlistIssue = (/** @type {string} */ message) => {
            try {
                logger_1.log.debug(`Source plist: ${lodash_1.default.truncate(JSON.stringify(plist), { length: 250 })}`);
            }
            catch { }
    ...
    decodeKey(obj, key) {
        const val = obj[key];
        return lodash_1.default.isNil(val?.UID) ? val : this.decodeObject(val);
    }
    decodeObject(index) {
        if (index === NULL_UID) {
            return null;
        ...
        if (!lodash_1.default.isUndefined(obj)) {
            return obj;
        }
...
class Archive {
    ...
    uidForArchiver(archiver, ...addition) {
        // @ts-ignore UID is not exposed to typedefs
        const val = new bplist_parser_1.default.UID(this.objects.length);
        this.objects.push({
    ...
    archive(obj) {
        if (lodash_1.default.isUndefined(obj) || lodash_1.default.isNull(obj)) {
            return NULL_UID;
        }
        // @ts-ignore UID is not exposed to typedefs
        const index = new bplist_parser_1.default.UID(this.objects.length);
    ...
    encodeTopLevel(obj, archiveObj) {
        if (obj instanceof Array) {
            ...
            throw new Error(`Unable to encode types: ${(0, node_util_1.format)('%O', obj)}`);
        }
    }
    ...
    encodeDict(obj, archiveObj) {
        archiveObj['NS.keys'] = lodash_1.default.keys(obj).map(this.archive.bind(this));
        archiveObj['NS.objects'] = lodash_1.default.values(obj).map(this.archive.bind(this));
    ...
    encodeXCTestConfiguration(obj, archiveObj) {
        ...
        lodash_1.default.entries(obj._data).forEach(([key, value]) => (archiveObj[key] = key === FORMAT_VERSION ? this.archive(value) : this.encode(value)));
    }
    toBytes() {
        ...
        const d = {
            ...
            // @ts-ignore UID is not exposed to typedefs
            $top: { root: new bplist_parser_1.default.UID(1) },
            $objects: this.objects,
        };
        return (0, bplist_creator_1.default)(d);
    }
}
/**
...
function updateNSKeyedArchiveClass(name, subClass) {
    ...
    !lodash_1.default.isFunction(subClass.prototype?.decodeArchive) &&
        // @ts-ignore prototype always exists
        !lodash_1.default.isFunction(subClass.prototype?.encodeArchive)) {
...
//# sourceMappingURL=nskeyed.js.map