...
exports.LengthBasedSplitter = void 0;
...
class LengthBasedSplitter extends node_stream_1.default.Transform {
    constructor(opts) {
        super();
    ...
    _decode(data, pos) {
        ...
        if (DEBUG_CONFIG.enableFrameSizeLogging) {
            logger_1.log.debug(`Frame size detected: ${messageLength} bytes, max allowed: ${this.maxFrameLength} bytes`);
        }
    ...
    _pushBack(start, end, data) {
        if (start > end) {
            logger_1.log.error('More data was read than the buffer size. This should not happen');
        }
        if (start === end) {
            return;
        }
    ...
    _logLargeFrame(messageLength) {
        logger_1.log.warn(`Large frame detected: ${support_1.util.toReadableSizeString(messageLength)}`);
        logger_1.log.warn(`Service: ${this.serviceName}, Max allowed: ${support_1.util.toReadableSizeString(this.maxFrameLength)}`);
        // Log memory usage if monitoring is enabled
        if (DEBUG_CONFIG.enableMemoryMonitoring) {
            const memUsage = process.memoryUsage();
            logger_1.log.warn(`Memory usage - RSS: ${support_1.util.toReadableSizeString(memUsage.rss)}, Heap: ${support_1.util.toReadableSizeString(memUsage.heapUsed)}`);
        }
    }
    /**
    ...
    _makeFrameSizeError(messageLength) {
        const errorMsg = `The frame is bigger than expected. ` +
            `Length: ${support_1.util.toReadableSizeString(messageLength)}, ` +
            `max: ${support_1.util.toReadableSizeString(this.maxFrameLength)}`;
        logger_1.log.error(errorMsg);
        logger_1.log.error(`Service: ${this.serviceName}, ` +
        ...
        if (!DEBUG_CONFIG.enableFrameSizeLogging) {
            logger_1.log.error('To debug this issue, see: https://github.com/appium/appium-ios-device/blob/master/docs/DEBUG_FRAME_SIZE.md');
        }
...
exports.default = LengthBasedSplitter;
//# sourceMappingURL=length-based-splitter.js.map