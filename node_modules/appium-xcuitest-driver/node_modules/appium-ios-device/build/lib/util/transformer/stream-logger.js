...
class StreamLogger extends node_stream_1.default.Transform {
    ...
    _transform(data, encoding, onData) {
        if (this._verbose) {
            try {
                ...
                logger_1.log.debug(`Error logging data: ${err.message}`);
            }
        }
        this.push(data);
        onData();
    ...
    _log(data) {
        logger_1.log.debug(`Data ${this._direction === StreamLogger.SEND ? 'sent' : 'received'}, length: ${data.length}`);
        ...
        while (i < data.length) {
            ...
            logger_1.log.debug(`${hex.join(' ')} ${char.join('')}`);
        }
    }
}
