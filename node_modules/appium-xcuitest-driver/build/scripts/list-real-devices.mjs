...
class RealDevicesLister {
    ...
    async run(opts) {
        ...
        log.info(`${util.pluralize('unique device', grouped.length, true)} ` +
    ...
    async _listFilteredUsbmux(connection) {
        ...
        if (raw.length === 0) {
            log.info('No devices reported by usbmuxd. Connect an unlocked device with a data-capable USB cable, ' +
        ...
        if (filtered.length === 0 && raw.length > 0) {
            log.info(`No devices match connection=${connection}; usbmuxd reported ` +
    ...
    async _listFilteredDevicectl(connection) {
        ...
        if (filtered.length === 0 && raw.length > 0) {
            log.info(`No devices match connection=${connection}; devicectl reported ` +
                `${util.pluralize('device', raw.length, true)} in total.`);
        }
        return filtered;
    }
...
async function main() {
    ...
    program
        ...
        .description('List real devices via usbmuxd by default, or via xcrun devicectl with --devicectl. ' +
        ...
        .addOption(new Option('--connection <type>', 'filter by connection (usbmux: USB vs Network; devicectl: wired/usb vs wireless e.g. localNetwork)')
 ...
 */
//# sourceMappingURL=list-real-devices.mjs.map