...
import { AppleTVTunnelService, PacketStreamServer, TunnelManager, createLockdownServiceByUDID, createUsbmux, startCoreDeviceProxy, startTunnelRegistryServer, TUNNEL_CONTAINER_NAME, } from 'appium-ios-remotexpc';
...
class TunnelCreator {
    ...
    async cleanup() {
        ...
        const closeUsbPacketStreamServers = (async () => {
            ...
            await Promise.allSettled(usbEntries.map(async ([udid, server]) => {
                try {
                    await server.stop();
        ...
        const closeAppleTVTunnels = (async () => {
            ...
            await Promise.allSettled(appletvResources.map(async (resource) => {
    ...
    async setupAppleTVTunnels(specificDeviceId) {
        ...
        try {
            ...
            const result = await tunnelService.startTunnel(undefined, specificDeviceId ?? undefined);
            ...
            if (tunnelService) {
                await teardownAppleTVTunnelResource({ tunnel, packetStreamServer, tunnelService, tlsSocket }, 'partially created');
            }
            return entries;
        }
    }
...
async function main() {
    ...
    program
        ...
        .option('--packet-stream-base-port <port>', 'Base port for packet stream servers (1-65535)', (value) => {
        const port = Number.parseInt(value, 10);
        if (!Number.isFinite(port) || port <= 0 || port > 65535) {
            throw new Error(`Invalid packet stream base port: ${value}. Expected an integer between 1 and 65535.`);
        }
        return port;
    })
        ...
        const port = Number.parseInt(value, 10);
        if (!Number.isFinite(port) || port <= 0 || port > 65535) {
            throw new Error(`Invalid tunnel registry port: ${value}. Expected an integer between 1 and 65535.`);
        }
        return port;
    })
        .option('--appletv-device-id <identifier>', 'Apple TV device identifier to tunnel (from pair-appletv); omit to use first discovered paired device');
