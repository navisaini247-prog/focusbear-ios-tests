...
const deviceInfoCommands = __importStar(require("./commands/device-info"));
...
class XCUITestDriver extends driver_1.BaseDriver {
    ...
    _perfRecorders;
    webElementsCache;
    _conditionInducer; // Condition inducer facade that abstracts implementation details
    _isSafariIphone;
    _isSafariNotched;
    _waitingAtoms;
    lifecycleData;
    ...
    constructor(opts, shouldValidateCaps = true) {
        ...
        this.deviceConnectionsFactory = new device_connections_factory_1.DeviceConnectionsFactory(this.log);
    ...
    async createSession(w3cCaps1, w3cCaps2, w3cCaps3, driverData) {
        try {
            ...
            if ('elementResponseAttributes' in this.opts &&
                lodash_1.default.isString(this.opts.elementResponseAttributes)) {
                wdaSettings.elementResponseAttributes = this.opts.elementResponseAttributes;
            }
            if ('shouldUseCompactResponses' in this.opts &&
                lodash_1.default.isBoolean(this.opts.shouldUseCompactResponses)) {
                wdaSettings.shouldUseCompactResponses = this.opts.shouldUseCompactResponses;
            }
            if ('mjpegServerScreenshotQuality' in this.opts &&
                lodash_1.default.isNumber(this.opts.mjpegServerScreenshotQuality)) {
    ...
    validateDesiredCaps(caps) {
        ...
        if (caps.ignoredWebviewBundleIds) {
            caps.ignoredWebviewBundleIds = this.helpers.parseCapsArray(caps.ignoredWebviewBundleIds);
        }
    ...
    async startWda() {
        ...
        await this.deviceConnectionsFactory.requestConnection(this.opts.udid, Number(this.wda.url.port), {
        ...
        if (this.opts.usePreinstalledWDA &&
            ...
            !(await support_1.fs.exists(this.opts.prebuiltWDAPath))) {
        ...
        if (this.opts.udid) {
            if (this.opts.udid.toLowerCase() === utils_1.UDID_AUTO) {
                ...
                if (shouldCheckAvailableRealDevices) {
                    const devices = await (0, real_device_management_1.getConnectedDevices)(this.opts);
            ...
            const device = new real_device_management_1.RealDevice(this.opts.udid, this.opts, this.log);
    ...
    async checkAutInstallationState(opts) {
        ...
        const appBundleVersion = (this.isRealDevice()
            ? await this.device.fetchAppInfo(bundleId)
            : await this.device.simctl.appInfo(bundleId))?.CFBundleVersion;
