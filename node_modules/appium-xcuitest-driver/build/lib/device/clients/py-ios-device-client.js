...
class Pyidevice extends base_device_client_1.BaseDeviceClient {
    ...
    async installProfile(args) {
        ...
        try {
            if (!srcPath) {
                ...
                srcPath = node_path_1.default.join(tmpRoot, 'cert.pem');
    ...
    async removeProfile(name) {
        return (await this.execute(['profiles', 'remove', '--name', name], {
            logStdout: true,
        })).stdout;
    }
    async listCrashes() {
        ...
        // 'Assistant']
        return JSON.parse(stdout.replace(/'/g, '"')).filter((x) => x.endsWith(CRASH_REPORT_EXT));
    }
