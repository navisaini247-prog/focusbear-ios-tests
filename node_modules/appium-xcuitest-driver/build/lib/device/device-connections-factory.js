...
class DeviceConnectionsFactory {
    ...
    listConnections(udid = null, port = null, strict = false) {
        ...
        return lodash_1.default.keys(DeviceConnectionsFactory._connectionsMapping).filter((key) => strict && udid && port
    ...
    async requestConnection(udid, port, options = {}) {
        ...
        this.log.debug(`Cached connections count: ${lodash_1.default.size(DeviceConnectionsFactory._connectionsMapping)}`);
        ...
        if (usePortForwarding) {
            ...
            await this._startAndRegisterPortForwarder(currentKey, udid, port, Number(devicePort), platformVersion);
        }
        else {
    ...
    async releaseConnection(udid = null, port = null) {
        if (!udid && !port) {
            this.log.warn('Neither device UDID nor local port is set. ' +
        ...
        if (isPortBusy) {
            ...
            if (!lodash_1.default.isEmpty(connectionsOnPort)) {
                ...
                try {
                    ...
                    this.log.warn(`Did not know how to release port #${port} in ` +
                        `${timer.getDuration().asMilliSeconds.toFixed(0)}ms`);
                }
            }
        }
    ...
    async _startAndRegisterPortForwarder(currentKey, udid, port, devicePort, platformVersion) {
    ...
    async _createPortForwarder(udid, localPort, devicePort, platformVersion) {
        if (!(0, utils_1.isIos18OrNewerPlatform)(platformVersion)) {
            this.log.debug(`Device '${udid}' is running iOS below 18 (platformVersion='${platformVersion ?? 'unknown'}'). ` +
                `Using appium-ios-device port forwarding fallback.`);
            return new LegacyPortForwarder(udid, localPort, devicePort, this.log);
        }
        let remotexpc = null;
        try {
            remotexpc = (await import('appium-ios-remotexpc'));
        }
        catch (err) {
            this.log.debug(`appium-ios-remotexpc is unavailable. Falling back to appium-ios-device port forwarding. ` +
                `Original error: ${err.message}`);
        }
        if (!remotexpc) {
            return new LegacyPortForwarder(udid, localPort, devicePort, this.log);
        }
        const listedByUsbmux = await (0, usbmux_utils_1.isDeviceListedInUsbmux)(remotexpc, udid, this.log);
        if (listedByUsbmux) {
            this.log.debug(`Using appium-ios-remotexpc usbmux strategy for '${udid}'`);
            return new RemotexpcPortForwarder(new remotexpc.DevicePortForwarder(localPort, devicePort, {
                primaryConnector: () => remotexpc.connectViaUsbmux(udid, devicePort),
            }), this.log, localPort, devicePort);
        }
        ...
        this.log.debug(`Using appium-ios-remotexpc tunnel strategy for '${udid}' through '${tunnelHost}'`);
        return new RemotexpcPortForwarder(new remotexpc.DevicePortForwarder(localPort, devicePort, {
...
class RemotexpcPortForwarder {
    ...
    onAbsorbForwarderError = () => { };
    onUpstreamConnectError = (err) => {
        const msg = err instanceof Error ? err.message : String(err);
        this.log.debug(`RemoteXPC port forwarder upstream connect error (local ${this.localPort} -> device ${this.devicePort}): ${msg}`);
    };
