...
exports.mobileStartXctestScreenRecording = mobileStartXctestScreenRecording;
exports.mobileGetXctestScreenRecordingInfo = mobileGetXctestScreenRecordingInfo;
...
async function retrieveRecodingFromSimulator(uuid) {
    ...
    if (lodash_1.default.isEmpty(videoPaths)) {
...
async function mobileStartXctestScreenRecording(fps, codec) {
    ...
    if (lodash_1.default.isInteger(fps)) {
        opts.fps = fps;
    }
    const response = (await this.proxyCommand('/wda/video/start', 'POST', opts));
 ...
 */
async function mobileGetXctestScreenRecordingInfo() {
    return (await this.proxyCommand('/wda/video', 'GET'));
}
/**
 ...
 */
async function mobileStopXctestScreenRecording(remotePath, user, pass, headers, fileFieldName, formFields, method = 'PUT') {
