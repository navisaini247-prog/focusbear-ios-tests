...
const certificate_client_1 = require("../device/certificate-client");
...
async function mobileInstallCertificate(content, commonName, isRoot = true) {
    ...
    const configPath = node_path_1.default.resolve(tmpRoot, configName);
    const tmpServer = node_http_1.default.createServer(async function (_, res) {
...
async function mobileRemoveCertificate(name) {
    (0, utils_1.requireRealDevice)(this, 'Removing certificate');
...
async function mobileListCertificates() {
    (0, utils_1.requireRealDevice)(this, 'Listing certificates');
 ...
 */
async function withCertificateClient(driver, operation) {
    let client = null;
    try {
        if (!driver.opts.udid) {
            throw new Error('udid capability is required');
        }
        client = await certificate_client_1.CertificateClient.create(driver.opts.udid, driver.log, (0, utils_1.isIos18OrNewer)(driver.opts));
...
function toMobileConfig(certBuffer, commonName) {
    ...
    return {
        ...
        PayloadDisplayName: commonName,
        PayloadIdentifier: `${node_os_1.default.hostname().split('.')[0]}.${getUUID()}`,
