...
class AndroidSettings {
    ...
    private get platformVersion(): number {
        return parseInt(
            (('platformVersion' in driver.capabilities &&
                driver.capabilities.platformVersion) as unknown as string) || '8',
            10,
        );
    }
     ...
     * Find an Android element based on text
     */
    async findAndroidElementByClassAndText(androidClass: string, string: string) {
        const selector = `android=new UiSelector().className("${androidClass}").textContains("${string}")`;
